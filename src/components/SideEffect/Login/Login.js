...
const Login = ({ onLogin }) => {
  ...
  useEffect(() => {
    // setTimeout을 사용해서 유혀성 검증을 1초 뒤에 실행하도록 작성
    // 1초 이내에 새로운 입력값이 들어온다면 -> 상태변경 -> useEffect가 다시 실행됨
    const timer = setTimeout(() => {
      setFormIsValid(emailIsValid && enteredPw.trim().length > 6);
    }, 1000);

    // cleanup 함수: 컴포넌트가 업데이트 되거나, 없어지기 직전에 실행
    // 사용자가 1초 이내에 추가 입력 -> 상태변경 (컴포넌트 업데이트) -> 위에 예약한 timer를 취소
    return () => {
      clearTimeout(timer);
    };
    // 의존성 배열에 상태변수를 넣어주면 그 상태변수가 변경될 때마다 useEffet가 재실행됨
