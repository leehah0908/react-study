...
const ExpenseItem = ({ title, price, date }) => {
  // 값이 변경되어서 화면에 반영되어야 하는 값들은 useState 훅을 통해 상태변수고 관리해야 합니다.
  // 훅: React에서 직접 코드 작성없이 다양한 기능을 사용할 수 있게 도와주는 라이브러리

  // useState(상태변수의 초기값): 배열을 리턴합니다.
  // 첫번째 요소는 관리할 상태값
  // 두번째 요소는 상태값을 변경해주는 setter 함수가 리턴됨 (디스트럭처링으로 한번에 받음)
  const [itemTtile, setItemTitle] = useState(title);
