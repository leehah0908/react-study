...
import Home from './components/SideEffect/Home/Home.js';

const App = () => {
  // 로그인 상태를 기억하는 변수
  const [isLoggedIn, setIsLoggedIn] = useState(false);

  // 화면이 리랜더링될 때 localStorage를 확인해서 현재 login-flag 데이터가 존재하는지 확인
  ...
  // 배열 안에 상태 변수를 지정하면, 해당 변수의 값이 변할 때마다 useEffect가 실행되고, 만약 빈 배열을 전달하면 최초 렌더링 과정에서 단 한번만 실행

  // 서버로 로그인을 요청하는 함수 (나중에 비동기 방식으로 실제 요청이 들어갈 것)
  const loginHandler = (email, password) => {
    if (email === 'abc1234@naver.com' && password === 'aaa1111') {
      // 로그인 성공
      // 로그인을 했다는 증거로 상태값 변경 및 브라우저에 로그인값을 1로 저장
      setIsLoggedIn(true);
      // 브라우저가 제공하는 저장소. 새로고침이나 브라우저를 종료해도 대아터가 계속 유지됨.
